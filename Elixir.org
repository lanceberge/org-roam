...
** Changesets
   ...
   - Changesets are essentially a builder for queries. You pass attributes to them and they go through each of the validation steps
     then you can pipe it into Repo.insert() and it will fail if it's invalid
   - You can pipe changesets into other changesets to add additional rules. The common pattern is to have in your model something like
     ~User.email_changeset~ that provides validation just for the emails. Then your context would execute the query, e.g.:
